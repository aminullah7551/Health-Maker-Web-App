...
Class MemberRegistrationController  extends AppController
{
	...
	{		
		...
		$member = $this->MemberRegistration->HealthMember->newEntity();
		$m = date("d");
		$y = date("y");
		$prefix = "M".$lastid;
		...
		$classes = $this->MemberRegistration->HealthMember->ClassSchedule->find("list",["keyField"=>"id","valueField"=>"class_name"]);
		$groups = $this->MemberRegistration->HealthMember->HealthGroup->find("list",["keyField"=>"id","valueField"=>"name"]);
		$interest = $this->MemberRegistration->HealthMember->HealthInterestArea->find("list",["keyField"=>"id","valueField"=>"interest"]);
		$source = $this->MemberRegistration->HealthMember->HealthSource->find("list",["keyField"=>"id","valueField"=>"source_name"]);
		$membership = $this->MemberRegistration->HealthMember->Membership->find("list",["keyField"=>"id","valueField"=>"membership_label"]);
		...
		if($this->request->is("post"))
		{
			...
			$member = $this->MemberRegistration->HealthMember->patchEntity($member,$this->request->data);
			
			if($this->MemberRegistration->HealthMember->save($member))
			{
				...
				if(!empty($this->request->data["assign_class"]))
				{
					...
					{
						$new_row = $this->MemberRegistration->HealthMemberClass->newEntity();
						$data = array();
						$data["member_id"] = $member->id;
						$data["assign_class"] = $class;
						$new_row = $this->MemberRegistration->HealthMemberClass->patchEntity($new_row,$data);
						$this->MemberRegistration->HealthMemberClass->save($new_row);
					}
				}
