...
class InstallerController extends AppController
{
	public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['index',"healthTableInstall","success","updateSys"]);
    }

	public function initialize()
    {
        parent::initialize();
		$this->viewBuilder()->layout("health_install");
	...
	public function index() {
		...
		if (file_exists(TMP.'installed.txt')) {
			...
			if($this->request->is("post")) {
				...
				$this->healthTableInstall($db_name,$db_username,$db_host,$db_pass);
				$this->insertData($this->request->data);
			}
		}
	}

	private function healthTableInstall($db_name,$db_username,$db_host,$db_pass)
    {
...
/* 		$sql = "CREATE DATABASE IF NOT EXISTS `{$db_name}` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci";
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_accessright` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$insert ="INSERT INTO `health_accessright` (`controller`, `action`, `menu`, `menu_icon`, `menu_title`, `member`, `staff_member`, `accountant`, `page_link`) VALUES
					...
					('HealthGroup', '', 'group', 'group.png', 'Group', 1, 1, 0, '".$path."/health-group/group-list'),
					('HealthMember', '', 'member', 'member.png', 'Member', 1, 1, 1, '".$path."/health-member/member-list'),
					...
					('HealthAttendance', '', 'attendance', 'attendance.png', 'Attendance', 0, 1, 0, '".$path."/health-attendance/attendance'),
					('HealthAssignWorkout', '', 'assign-workout', 'assigne-workout.png', 'Assigned Workouts', 1, 1, 0, '".$path."/health-assign-workout/workout-log'),
					('HealthDailyWorkout', '', 'workouts', 'workout.png', 'Workouts', 1, 1, 0, '".$path."/health-daily-workout/workout-list'),
					('HealthAccountant', '', 'accountant', 'accountant.png', 'Accountant', 1, 1, 1, '".$path."/health-accountant/accountant-list'),
					...
					('HealthProduct', '', 'product', 'products.png', 'Product', 0, 1, 1, '".$path."/health-product/product-list'),
					('HealthStore', '', 'store', 'store.png', 'Store', 0, 1, 1, '".$path."/health-store/sell-record'),
					('HealthNewsletter', '', 'news_letter', 'newsletter.png', 'Newsletter', 0, 0, 0, '".$path."/health-newsletter/setting'),
					('HealthMessage', '', 'message', 'message.png', 'Message', 1, 1, 1, '".$path."/health-message/compose-message'),
					('HealthNotice', '', 'notice', 'notice.png', 'Notice', 1, 1, 1, '".$path."/health-notice/notice-list'),
					('HealthNutrition', '', 'nutrition', 'nutrition-schedule.png', 'Nutrition Schedule', 1, 1, 0, '".$path."/health-nutrition/nutrition-list'),
					('HealthReservation', '', 'reservation', 'reservation.png', 'Event', 1, 1, 1, '".$path."/health-reservation/reservation-list'),
					('HealthProfile', '', 'account', 'account.png', 'Account', 1, 1, 1, '".$path."/HealthProfile/view_profile'),
					('HealthSubscriptionHistory', '', 'subscription_history', 'subscription_history.png', 'Subscription History', 1, 0, 0, '".$path."/HealthSubscriptionHistory/')";

		$stmt = $conn->execute($sql);
		$stmt = $conn->execute($insert);

		$sql="CREATE TABLE IF NOT EXISTS `health_assign_workout` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
			  `user_id` int(11) NULL,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_attendance` (
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_daily_workout` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_event_place` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_group` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_income_expense` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_interest_area` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_levels` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_measurement` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_member` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$insert = "INSERT INTO `health_member` (`role_name`,`first_name`, `middle_name`, `last_name`,`gender`, `birth_date`,`address`, `city`, `state`, `zipcode`, `mobile`, `phone`, `email`,`username`, `password`, `image`,`created_date`) VALUES
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_member_class` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
			  `member_id` int(11) NULL,
			  `assign_class` int(11) NULL,
			  PRIMARY KEY (`id`)
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_message` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
			  `sender` int(11) NULL,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_newsletter` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_notice` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_nutrition` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
			  `user_id` int(11) NULL,
			  `day` varchar(50) NULL,
			  `breakfast` text NULL,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_nutrition_data` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_product` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_reservation` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_roles` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
			  `name` varchar(100) NULL,
			  PRIMARY KEY (`id`)
				)DEFAULT CHARSET=utf8";
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_source` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_store` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
			  `member_id` int(11) NULL,
			  `sell_date` date NULL,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_user_workout` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql="CREATE TABLE IF NOT EXISTS `health_workout_data` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		...
		$sql = "ALTER TABLE `health_member` ADD `stripe_customer_id` VARCHAR(255) NULL DEFAULT NULL AFTER `member_id`";
		$stmt = $conn->execute($sql);

		$sql = "ALTER TABLE `health_member` ADD `alert_send_date` DATE NULL AFTER `alert_sent`";
			$conn->execute($sql);

		$sql = "ALTER TABLE `health_member` ADD `admin_alert` INT NULL DEFAULT '0' AFTER `alert_sent`";
			$conn->execute($sql);
		...
		$path = $this->request->base;
		$sql = "INSERT INTO `health_accessright` (`controller`, `action`, `menu`, `menu_icon`, `menu_title`, `member`, `staff_member`, `accountant`, `page_link`) VALUES ('Reports', '', 'report', 'report.png', 'Report', '1', '1', '1', '".$path."/reports/membership-report')";
			$conn->execute($sql);

		$sql = "ALTER TABLE `general_setting` ADD `time_zone` VARCHAR(20) NOT NULL DEFAULT 'UTC' AFTER `datepicker_lang`";
			$conn->execute($sql);

		$sql = "ALTER TABLE `health_member` ADD `token` VARCHAR(300) NULL DEFAULT NULL AFTER `member_id`";
			$conn->execute($sql);
	...
	private function insertData($data)
	{
		...
		$year = date("Y");
		$conn = ConnectionManager::get('install_db');
		$sql  = "INSERT INTO `general_setting` (`name`, `start_year`, `address`, `office_number`, `country`, `email`, `date_format`, `calendar_lang`, `health_logo`, `cover_image`, `weight`, `height`, `chest`, `waist`, `thing`, `arms`, `fat`, `member_can_view_other`, `staff_can_view_own_member`, `enable_sandbox`, `paypal_email`, `currency`, `enable_alert`, `reminder_days`, `reminder_message`, `enable_message`, `left_header`, `footer`,`system_installed`,`datepicker_lang`,`sys_language`,`header_color`,`sidemenu_color`,`stripe_secret_key`,`stripe_publishable_key`) VALUES
			('{$data['name']}', '{$year}', 'address', '8899665544', '{$data['country']}','{$data['email']}', '{$data['date_format']}', '{$data['sys_language']}', '', 'cover-image.png', 'KG', 'Centimeter', 'Inches', 'Inches', 'Inches', 'Inches', 'Percentage', 0, 1, 0, 'your_id@paypal.com', '{$data['currency']}', 1, '5', 'Hello HEALTH_MEMBERNAME,\r\n      Your Membership  HEALTH_MEMBERSHIP  started at HEALTH_STARTDATE and it will expire on HEALTH_ENDDATE.\r\nThank You.', 1,'HEALTH MASTER','Copyright © 2016-2017. All rights reserved.',1,'{$data['sys_language']}','{$data['sys_language']}','#1DB198','','YOUR SECRET KEY','YOUR PUBLISHABLE KEY')";
		$stmt = $conn->execute($sql);
		...
		$sql = "INSERT INTO `category` (`name`) VALUES
				('Regular'),
				('Limited'),
				('Total Health Exercises for Abs (Abdomininals)'),
				('Total Health Exercises for Legs'),
				('Total Health Exercises for Biceps'),
				('Exercise')";
		$stmt = $conn->execute($sql);
		...
		$sql = "INSERT INTO `health_roles` (`name`) VALUES
				('Yoga')";
		$stmt = $conn->execute($sql);
		...
		$sql = "INSERT INTO `class_schedule` (`class_name`, `assign_staff_mem`, `assistant_staff_member`, `location`,`class_fees`, `days`, `start_time`, `end_time`, `created_by`, `created_date`) VALUES
				('Yoga Class', 2, 0, 'At Health Facility','5', '[\"Sunday\",\"Saturday\"]', '8:00', '10:00' ,1, '2016-08-22'),
				...
				('Cardio Class', 2, 0, 'At Health Facility', '5', '[\"Friday\",\"Saturday\"]', '15:30', '16:30',1, '2016-08-22'),
	...
	public function updateSys()
	{
		...
		if(!empty($settings))
		{
			if(isset($settings[0]["system_version"]))
			{
				...
				switch($version)
				{
					...
					CASE "13":
						...
						$path = $this->request->base;
						$insert ="INSERT INTO `health_accessright` (`controller`, `action`, `menu`, `menu_icon`, `menu_title`, `member`, `staff_member`, `accountant`, `page_link`) VALUES ('ClassBooking', '', 'class-booking', 'class-schedule.png', 'Class Booking', 0, 1, 0, '".$path."/class-booking/booking-list')";
			...
			else
			{
				...
				// $sql = "ALTER TABLE `health_daily_workout` ADD `reminder_status` TINYINT NOT NULL DEFAULT '0' AFTER `status`";
				// $conn->execute($sql);
