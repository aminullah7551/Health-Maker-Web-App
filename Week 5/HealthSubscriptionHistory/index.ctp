		...
		<div class="box-body">
			...
			<tbody>
				...
				if(!empty($data))
				{
					...
					{
						echo "<tr>
							<td>".$this->Health->get_member_name($row['member_id'])."</td>
							<td>{$row["Membership"]["membership_label"]}</td>
							<td>{$row["membership_amount"]}</td>
							<td>". ($row["membership_amount"] - $row["paid_amount"]) ."</td>
							<td>".$this->Health->get_db_format(date($this->Health->getSettings("date_format"),strtotime($row["start_date"])))."</td>
							<td>".$this->Health->get_db_format(date($this->Health->getSettings("date_format"),strtotime($row["end_date"])))."</td>
