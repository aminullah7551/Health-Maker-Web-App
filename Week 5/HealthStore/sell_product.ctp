...
$(document).ready(function(){
	$(".sell-date").datepicker( "option", "dateFormat", "<?php echo $this->Health->dateformat_PHP_to_jQueryUI($this->Health->getSettings("date_format")); ?>" );
	<?php
	if($edit){?>
	$( ".sell-date" ).datepicker( "setDate", new Date("<?php echo date($this->Health->getSettings("date_format"),strtotime($data['sell_date'])); ?>" ));
	<?php } ?>
	$(".mem_list").select2({
	...
	<div class="col-md-12 box box-default">		
		<div class="box-header">
			<section class="content-header">
			  ...
			  <ol class="breadcrumb">
				<a href="<?php echo $this->Health->createurl("HealthStore","sellRecord");?>" class="btn btn-flat btn-custom"><i class="fa fa-bars"></i> <?php echo __("Sell Records");?></a>
			 </ol>
			</section>
		</div>
		<hr>
		...
		<div class="col-md-6">
			<input type="text" autocomplete="off" name="sell_date" class="sell-date form-control validate[required]" value="<?php echo ($edit)?date($this->Health->getSettings("date_format"),strtotime($data["sell_date"])) : "";?>">
		</div>	
		</div>
		<div class='form-group'>	
		...
		{?>
		<div class="col-md-3">
			<a href="<?php echo $this->request->base ."/HealthProduct/addProduct";?>" class="btn btn-flat btn-default"><?php echo __("Add Product");?></a>
		</div>
	<?php } ?>
		</div>
		<div class='form-group'>	
