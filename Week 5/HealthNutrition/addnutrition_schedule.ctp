	...
	<div class="col-md-12 box box-default">		
		<div class="box-header">
			<section class="content-header">
			  ...
			  <ol class="breadcrumb">
				<a href="<?php echo $this->Health->createurl("HealthNutrition","nutritionList");?>" class="btn btn-flat btn-custom"><i class="fa fa-bars"></i> <?php echo __("Nutrition Schedule List");?></a>
			  </ol>
			</section>
		</div>
		<hr>
		...
		<div class='form-group'>
			...
			<div class="health_addform col-md-6 col-sm-9">
				<?php 	
			...
			?>
				<div class="panel panel-default workout-block" id="remove_panel_<?php echo $data;?>">				
				  <div class="panel-heading">
					<i class="fa fa-calendar"></i> <?php echo __("Start From")." <span class='work_date'>". $this->Health->get_db_format(date($this->Health->getSettings("date_format"),strtotime($row['start_date'])))."</span> ".__("TO")." <span class='work_date'>".$this->Health->get_db_format(date($this->Health->getSettings("date_format"),strtotime($row['expire_date'])))."</span>";?>
					<span class="del_nutrition_panel" del_id="<?php echo $data;?>" data-url="<?php echo $this->request->base;?>/HealthAjax/deleteNutritionData/<?php echo $data;?>"><i class='fa fa-times-circle' aria-hidden="true"></i></span>
				  </div>
				  <br>
...
<script>
	...
	jQuery("body").on("click", "#add_nutrition", function(event) {
		...
		$.ajax({
			type : "POST",
			data : curr_data,
			url : "<?php echo $this->request->base . "/HealthAjax/health_add_nutrition"?>",
