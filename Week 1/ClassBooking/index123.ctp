...
<section class="content">
	...
	<div class="col-md-12 box box-default">		
		...
		<div class="panel">
		<?php				
			echo $this->Form->create("",["type"=>"file","class"=>"validateForm form-horizontal","role"=>"form"]);
			...
			echo $this->Form->select("class_id",$class,["default"=>($edit)?$data["class_id"]:"","empty"=>__("Select Class"),"class"=>"validation[required] form-control class",'required'=>'true','data-url'=>$this->request->base.'/ClassBooking/get_class_fees']);
			echo "</div>";			
			echo "</div>";
			...
			echo '<div class="col-md-6">';			
			echo $this->Form->select("booking_type",$book_type,["default"=>($edit)?$data["booking_type"]:"","empty"=>__("Select Booking Type"),"class"=>"validation[required] form-control book_type",'required'=>'true']);
			echo "</div>";			
			echo "</div>";
			...
			echo '<div class="col-md-6">';
			echo $this->Form->input("",["label"=>false,"name"=>"booking_date","class"=>"form-control dob validate[required] datepick","value"=>(($edit)?date($this->Health->getSettings("date_format"),strtotime($data['booking_date'])):''),"onkeydown"=>"return false"]);
			echo "</div>";
			echo "</div>";
			echo "</fieldset>";
			...
			echo $this->Form->button(__("Save Booking"),['class'=>"btn btn-flat btn-success","name"=>"save_booking"]);
			echo '<div class="stripe">
	        	 <script src="https://checkout.stripe.com/checkout.js" id="stripe" class="stripe-button"
		          data-key="'.$this->HEALTH->getSettings("stripe_publishable_key") .'"
...
$(document).ready(function(){
	...
	//$(".dob").datepicker({yearRange: "+0:+0",changeYear: true,changeMonth: true, dateFormat:"<?php echo $this->Health->dateformat_PHP_to_jQueryUI($this->Health->getSettings("date_format")); ?>" ,"language" : "<?php echo $dtp_lang;?>"});
	//$(".datepick").datepicker({format: 'yyyy-mm-dd',"language" : "<?php echo $dtp_lang;?>"});
	$(".datepick").datepicker({yearRange: "+0:+0",changeYear: true,changeMonth: true, dateFormat:"<?php echo $this->Health->dateformat_PHP_to_jQueryUI($this->Health->getSettings("date_format")); ?>" ,"language" : "<?php echo $dtp_lang;?>"});
