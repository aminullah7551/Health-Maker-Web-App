...
class DashboardController extends AppController
{
	...
	public function adminDashboard()
	{
		...
		$grp_tbl = TableRegistry::get("HealthGroup");
		$message_tbl = TableRegistry::get("HealthMessage");
		$membership_tbl = TableRegistry::get("Membership");				
		$notice_tbl = TableRegistry::get("health_notice");		
		...
		$cal_lang = $this->HEALTHFunction->getSettings("calendar_lang");
		...
		$report_2 ="SELECT  at.class_id,cl.class_name,
					...
					from `health_attendance` as at,`class_schedule` as cl where at.attendance_date >  DATE_SUB(NOW(), INTERVAL 1 WEEK) AND at.class_id = cl.id  AND at.role_name = 'member' GROUP BY at.class_id";
		...
		$report_2 ="SELECT  at.user_id,
				SUM(case when `status` ='Present' then 1 else 0 end) as Present,
				SUM(case when `status` ='Absent' then 1 else 0 end) as Absent
				from `health_attendance` as at where at.attendance_date >  DATE_SUB(NOW(), INTERVAL 1 WEEK)  AND at.role_name = 'staff_member' GROUP BY at.user_id";
		...
		if(!empty($report_2))
		{
			foreach($report_2 as $result)
			{
				$user_name = $this->HEALTHFunction->get_user_name($result["user_id"]);
		...
		$this->HEALTHFunction->membershipExpiredReminder();
	...
	public function memberDashboard()
	{
		...
		$message_tbl = TableRegistry::get("HealthMessage");
		$membership_tbl = TableRegistry::get("Membership");		
		$res_tbl = TableRegistry::get("health_reservation");		
		$notice_tbl = TableRegistry::get("health_notice");		
