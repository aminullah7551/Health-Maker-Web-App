		...
		public function login() {			
			if ($this->request->is('post')) {
				...
				if($users) {				
					...
					$logo = $this->HEALTHFunction->getSettings("health_logo");
					$logo = (!empty($logo)) ? "/webroot/upload/". $logo : "Thumbnail-img.png";
					$name = $this->HEALTHFunction->getSettings("name");
					$left_header = $this->HEALTHFunction->getSettings("left_header");
					$footer = $this->HEALTHFunction->getSettings("footer");
					$is_rtl = ($this->HEALTHFunction->getSettings("enable_rtl") == 1) ? true : false;
					$datepicker_lang = $this->HEALTHFunction->getSettings("datepicker_lang");
					$version = $this->HEALTHFunction->getSettings("system_version");
			...
			if ($this->request->is('post')) {
				...
				if(!empty($user)) {
					...
					if($this->Users->save($user)) {
						...
						$email->from(['healthmaster.cakephp@pushnifty.com' => 'HEALTH Master']);
