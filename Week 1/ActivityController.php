...

{
...

	{
...

		{
			...
			$mem_id = $this->Activity->HealthMember->find()->where(["id"=>$session["id"]])->select(["selected_membership"])->hydrate(false)->toArray();
...

			{
				...
				$data = $data->contain(["HealthMember","Category"])->select($this->Activity)->select(["HealthMember.first_name","HealthMember.last_name","Category.name"])->hydrate(false)->toArray();
			...
			$data = $this->Activity->find("all")->contain(["HealthMember","Category"])->select($this->Activity)->select(["HealthMember.first_name","HealthMember.last_name","Category.name"])->hydrate(false)->toArray();
	...
	{		
		...
		$staff = $this->Activity->HealthMember->find("list",["keyField"=>"id","valueField"=>"name"])->where(["role_name"=>"staff_member"]);
		...
		{			
...

			{
				...
				if(!empty($this->request->data['video']) && $this->request->data['video'][0] != '' ){
	...
	public function editActivity($id) {
		...
		$role = $session["role_name"];
		$this->set("role",$role);
		$data = $this->Activity->find()->where(["Activity.id"=>$id])->contain(["Category","HealthMember"])->select($this->Activity);
		$data = $data->select(["HealthMember.first_name","HealthMember.last_name","Category.name"])->hydrate(false)->toArray();
