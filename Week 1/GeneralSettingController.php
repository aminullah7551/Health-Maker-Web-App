...
class GeneralSettingController extends AppController{
	...
	public function saveSetting()
	{
		...
		{
			$logo_ext = $this->HEALTHFunction->check_valid_extension($this->request->data['health_logo']['name']);
			$cover_ext = $this->HEALTHFunction->check_valid_extension($this->request->data['cover_image']['name']);
			...
			if($logo_ext != 0 && $cover_ext != 0)
			{
				...
				if($count == 0)
				{
					...
					$logo_name =  $this->HEALTHFunction->uploadImage($this->request->data['health_logo']);
					$cover_name =  $this->HEALTHFunction->uploadImage($this->request->data['cover_image']);
					$this->request->data['health_logo'] = $logo_name;
					$this->request->data['cover_image'] = $cover_name;
					$session->write("User.logo",$this->request->data["health_logo"]);
					...
					$logo_name =  $this->HEALTHFunction->uploadImage($this->request->data['health_logo']);
					$cover_name =  $this->HEALTHFunction->uploadImage($this->request->data['cover_image']);
					if($this->request->data['health_logo']['name'] != "")
					{
						$this->request->data['health_logo'] = $logo_name;
						$session->write("User.logo","/webroot/upload/".$this->request->data["health_logo"]);
					}else{
						$this->request->data['health_logo'] = $this->request->data['old_health_logo'];
					}
