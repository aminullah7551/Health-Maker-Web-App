...
class HealthNewsletterController extends AppController {
	public function initialize() {
		parent::initialize();
		require_once(ROOT . DS .'vendor' . DS  . 'mailchimp' . DS . 'MCAPI.class.php');
	}
	public function setting() {
		$key = $this->HealthNewsletter->find()->first();
		if($key != null) {
			$key = $key->toArray();
			$key = $key["api_key"];
			$this->set("key",$key);
		...
		if($this->request->is("post")) {
			if(!$edit) {
				$row = $this->HealthNewsletter->newEntity();
				$row = $this->HealthNewsletter->patchEntity($row,$this->request->data);
				if($this->HealthNewsletter->save($row)) {
					$this->Flash->success(__("Success! Api Key Saved successfully."));
					return $this->redirect($this->referer);
				}
			}else {
				$row = $this->HealthNewsletter->find()->first();
	...
	public function syncMail() {
		...
		if($this->request->is("post")) {
			...
			foreach ($syncmail as $id) {				
				$members = $mem_tbl->find("all")->where(["assign_class"=>$id,"role_name"=>"member"])->select(["HealthMember.first_name","HealthMember.last_name","HealthMember.email"])->hydrate(false)->toArray();
