	...
	<div class="col-md-12 box box-default">		
		<div class="box-header">
			<section class="content-header">
			  ...
			  <ol class="breadcrumb">
				<a href="<?php echo $this->Health->createurl("HealthDailyWorkout","workoutList");?>" class="btn btn-flat btn-custom"><i class="fa fa-bars"></i> <?php echo __("Workout List");?></a>
				&nbsp;
				<?php $role = $this->request->session()->read('User.role_name');
				if($role != 'accountant'){
					?>
					<a href="<?php echo $this->Health->createurl("HealthDailyWorkout","addMeasurment");?>" class="btn btn-flat btn-custom"><i class="fa fa-plus"></i> <?php echo __("Add Measurement");?></a>
				<?php } ?>
			  </ol>
			</section>
		</div>
        ...
        <div class="form-group">
			...
			<div class="col-sm-3 module_padding">				
				...
				<input type="text" name="schedule_date1" class="validate[required] date1 form-control" id="enabledate" value="<?php if(isset($_REQUEST['schedule_date'])){ echo $this->Health->get_db_format(date($this->Health->getSettings("date_format"),strtotime($_REQUEST['schedule_date'])));}else{echo "";}?>" autocomplete="off">
			...
			{ ?>
				...
				<div class="col-md-10 activity-data no-padding">
					<div class="workout_datalist_header">
						<h2><?php 
						if($workout['HealthUserWorkout']["workout_name"] != null) {
							echo $this->Health->get_activity_by_id($workout['HealthUserWorkout']["workout_name"]);
						}
						?></h2>
					</div>
					<div class="col-md-10 workout_datalist no-padding"> 
						<?php 
						$i = 1;
						for($i;$i<=$workout['HealthUserWorkout']["sets"];$i++)
						{ ?>
						<div class="col-md-6 sets-row no-paddingleft">	
							<span class="text-center sets_counter"><?php echo $i;?></span>
							<span class="sets_kg"><?php echo $workout['HealthUserWorkout']["kg"];?> Kg / Sets</span>								
							<span class="col-md-2 reps_count"><?php echo $workout['HealthUserWorkout']["reps"];?></span>
						</div>
				  <?php } ?>					
					</div>
