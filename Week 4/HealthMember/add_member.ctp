...
$(document).ready(function() {
	...
	$birthday = $date->format($this->Health->getSettings("date_format"));	
	//$birthday = $this->Health->dateformat_PHP_to_jQueryUI($this->Health->getSettings("date_format"));	
	...
	{
		?>
		$( ".inquiry_date" ).datepicker( "setDate", new Date("<?php echo date($this->Health->getSettings("date_format"),strtotime($inquiry_date)); ?>" )
		);
	...
	{ 
		?>
		$( ".trial_end_date" ).datepicker( "setDate", new Date("<?php echo date($this->Health->getSettings("date_format"),strtotime($trial_end_date)); ?>" ));
	...
	{?>
	$( ".first_pay_date" ).datepicker( "setDate", new Date("<?php echo date($this->Health->getSettings("date_format"),strtotime($first_pay_date)); ?>" ));
	<?php } ?>
			...
			$.ajax({
				...
				success : function(response)
				{
					...
					$(".valid_to,.check").datepicker({ language: "<?php echo $dtp_lang;?>",
						 dateFormat :"<?php echo $this->Health->dateformat_PHP_to_jQueryUI($this->Health->getSettings("date_format")); ?>",
	...
	<div class="col-md-12 box box-default">		
		<div class="box-header">
			<section class="content-header">
			  ...
			  <ol class="breadcrumb">
				<a href="<?php echo $this->Health->createurl("HealthMember","memberList");?>" class="btn btn-flat btn-custom"><i class="fa fa-bars"></i> <?php echo __("Members List");?></a>
			  </ol>
			</section>
		</div>
		<hr>
		...
		<?php	
			...
			echo '<div class="col-md-6">';
			echo $this->Form->input("",["label"=>false,"name"=>"birth_date","class"=>"dob form-control  validate[required] ","value"=>(($edit)?date($this->Health->getSettings("date_format"),strtotime($data['birth_date'])):''),"id"=>"birth_date","autocomplete"=>"off"]);
			echo "</div>";	
			echo "</div>";
			...
			echo "<a href='{$this->request->base}/HealthGroup/addGroup/' class='btn btn-flat btn-default'>".__("Add Group")."</a>";
			echo "</div>";	
			echo "</div>";
			echo "</fieldset>";
			...
			echo '<div class="input-group-addon">+'.$this->Health->getCountryCode($this->Health->getSettings("country")).'</div>';
			...
			echo '<div class="col-md-6">';
			echo $this->Form->input("",["label"=>false,"name"=>"weight","class"=>"form-control validate[custom[onlyNumberSp],maxSize[3]]","placeholder"=>$this->Health->getSettings(__("weight")),"value"=>(($edit)?$data['weight']:''),"id"=>"weight"]);
			echo "</div>";	
			echo "</div>";
			...
			echo '<div class="col-md-6">';
			echo $this->Form->input("",["label"=>false,"name"=>"height","class"=>"form-control validate[custom[onlyNumberSp],maxSize[3]]","placeholder"=>__($this->Health->getSettings("height")),"value"=>(($edit)?$data['height']:''),"id"=>"height"]);
			echo "</div>";	
			echo "</div>";	
			...
			echo '<div class="col-md-6">';
			echo $this->Form->input("",["label"=>false,"name"=>"chest","class"=>"form-control validate[custom[onlyNumberSp],maxSize[3]]","placeholder"=>__($this->Health->getSettings("chest")),"value"=>(($edit)?$data['chest']:''),"id"=>"chest"]);
			echo "</div>";	
			echo "</div>";	
			...
			echo '<div class="col-md-6">';
			echo $this->Form->input("",["label"=>false,"name"=>"waist","class"=>"form-control validate[custom[onlyNumberSp],maxSize[3]]","placeholder"=>__($this->Health->getSettings("waist")),"value"=>(($edit)?$data['waist']:''),"id"=>"waist"]);
			echo "</div>";	
			echo "</div>";	
			...
			echo '<div class="col-md-6">';
			echo $this->Form->input("",["label"=>false,"name"=>"thing","class"=>"form-control validate[custom[onlyNumberSp],maxSize[3]]","placeholder"=>__($this->Health->getSettings("thing")),"value"=>(($edit)?$data['thing']:''),"id"=>"thing"]);
			echo "</div>";	
			echo "</div>";	
			...
			echo '<div class="col-md-6">';
			echo $this->Form->input("",["label"=>false,"name"=>"arms","class"=>"form-control validate[custom[onlyNumberSp],maxSize[3]]","placeholder"=>__($this->Health->getSettings("arms")),"value"=>(($edit)?$data['arms']:''),"id"=>"arms"]);
			echo "</div>";	
			echo "</div>";	
			...
			echo '<div class="col-md-6">';
			echo $this->Form->input("",["label"=>false,"name"=>"fat","class"=>"form-control validate[custom[onlyNumberSp],maxSize[3]]","placeholder"=>__($this->Health->getSettings("fat")),"value"=>(($edit)?$data['fat']:''),"id"=>"fat"]);
			echo "</div>";	
			echo "</div>";	
			echo "</fieldset>";
			...
			echo "</div>";	
			echo '<div class="col-md-2">';
			echo "<a href='javascript:void(0)' class='btn btn-flat btn-default interest-list' data-url='{$this->request->base}/HealthAjax/interestList'>".__("Add/Remove")."</a>";
			echo "</div>";	
			echo "</div>";
			...
			echo "</div>";	
			echo '<div class="col-md-2">';
			echo "<a href='javascript:void(0)' class='btn btn-flat btn-default source-list' data-url='{$this->request->base}/HealthAjax/sourceList'>".__("Add/Remove")."</a>";
			echo "</div>";	
			echo "</div>";
			...
			echo '<div class="col-md-2">';
			echo $this->Form->input("",["autocomplete"=>"off","label"=>false,"name"=>"membership_valid_from","class"=>"form-control validate[required] date mem_valid_from","value"=>(($edit && $data['membership_valid_from']!="")?$this->Health->get_db_format(date($this->Health->getSettings("date_format"),strtotime($data['membership_valid_from']))):'')]);
			...
			echo "</div>";
			echo '<div class="col-md-2">';
			echo $this->Form->input("",["label"=>false,"name"=>"membership_valid","class"=>"form-control validate[required] valid_to","value"=>(($edit && $data['membership_valid_to']!="")?$this->Health->get_db_format(date($this->Health->getSettings("date_format"),strtotime($data['membership_valid_to']))):''),"disabled"=>true]);?>
			<input type='hidden' name='membership_valid_to' class='check' value='<?php echo ($edit && $data['membership_valid_to']!="")?$this->Health->get_db_format(date($this->Health->getSettings("date_format"),strtotime($data['membership_valid_to']))):''?>'>
			<?php echo "</div>";
			echo "</div>";
			...
			echo '<div class="col-md-6">';
			//echo $this->Form->input("",["label"=>false,"name"=>"first_pay_date","class"=>"form-control first_pay_date date","value"=>(($edit)?date($this->Health->getSettings("date_format"),strtotime($data['first_pay_date'])):'')]);
			...
			echo $this->Form->end();
		?>
		<input type="hidden" value="<?php echo $this->request->base;?>/HealthAjax/get_membership_end_date" id="mem_date_check_path">
		<input type="hidden" value="<?php echo $this->request->base;?>/HealthAjax/get_membership_classes" id="mem_class_url">
		</div>	
