...

{
	...
	public function qrReport(){
		...
		if($this->request->is("post")){
			...
			$att_tbl = TableRegistry::get("health_attendance");	

			$cls_tbl = TableRegistry::get("class_schedule");

			$sdate = $this->HEALTHFunction->get_db_format_date($this->request->data['sdate']);
			...
			//$report_2 = "SELECT  at.class_id,cl.class_name, at.scan_by from `health_attendance` as at,`class_schedule` as cl where at.attendance_date = '{$sdate}' AND at.role_name = 'member'";

			

			$report_2 = "select gm.first_name,gm.last_name,gt.class_id,cs.class_name,gms.first_name as memname,gms.last_name as memlastname from health_attendance gt INNER JOIN health_member gm ON gt.scan_by = gm.id INNER JOIN class_schedule cs ON gt.class_id = cs.id INNER JOIN health_member gms ON gt.user_id = gms.id WHERE gt.scan_date = '$sdate'";
	...
	public function membershipReport() {
		...
		$data = $data->leftjoin(["HealthMember"=>"health_member"],["HealthMember.selected_membership = Membership.id"])->select(["count"=>$data->func()->count("HealthMember.id")])->group("Membership.id")->hydrate(false)->toArray();
		if(!empty($data)) {
	...
	public function monthlyworkoutreport() {
		...
		if($this->request->is("post")) {
			...
			$conn = ConnectionManager::get('default');
			$query = "select * from health_daily_workout as gdw 
			LEFT JOIN health_user_workout as guw on guw.user_workout_id = gdw.id
			...
			if(isset($this->request->data["export_csv"])) {
				...
				$this->HEALTHFunction->export_to_csv($filename,$rows);
			}
		}
	}
	...
	public function attendanceReport() {
		...
		if($this->request->is("post")) {
			...
			$sdate = $this->HEALTHFunction->get_db_format_date($this->request->data['sdate']);
			$edate = $this->HEALTHFunction->get_db_format_date($this->request->data['edate']);
			$conn = ConnectionManager::get('default');	
			$report_2 = "SELECT  at.class_id,cl.class_name, 
				SUM(case when `status` ='Present' then 1 else 0 end) as Present, 
				SUM(case when `status` ='Absent' then 1 else 0 end) as Absent 
				from `health_attendance` as at,`class_schedule` as cl where `attendance_date` BETWEEN '{$sdate}' AND '{$edate}' AND at.class_id = cl.id AND at.role_name = 'member' GROUP BY at.class_id";
			...
			/* new added */
			$att_tbl = TableRegistry::get('health_attendance');
			$att = $att_tbl->find()->where(["attendance_date >= "=>$sdate,"attendance_date <= "=>$edate])->hydrate(false)->toArray();		
	...
	public function paymentReport() {			
		...
		$month =array('1'=> __("January"),'2'=> __("February"),'3'=> __("March"),'4'=> __("April"),
		'5'=> __("May"),'6'=> __("June"),'7'=> __("July"),'8'=> __("August"),
		'9'=> __("September"),'10'=> __("Octomber"),'11'=> __("November"),'12'=> __("December"),);
		$year = date('Y');
