...
class ClassScheduleController extends AppController
{
	...
	public function classList()
	{
		...
		{
			$classes = $this->ClassSchedule->HealthMemberClass->find()->where(["member_id"=>$session["id"]])->hydrate(false)->toArray();
			if(!empty($classes))
			{
				...
				$data = $data->contain(["HealthMember"])->select(["ClassSchedule.id","ClassSchedule.class_name","ClassSchedule.assign_staff_mem","ClassSchedule.start_time","ClassSchedule.end_time","ClassSchedule.location","ClassSchedule.class_fees","HealthMember.first_name","HealthMember.last_name"])->hydrate(false)->toArray();
			}
		}
		else{
			$data = $this->ClassSchedule->find();
			$data = $data->contain(["HealthMember"])->select(["ClassSchedule.id","ClassSchedule.class_name","ClassSchedule.assign_staff_mem","ClassSchedule.start_time","ClassSchedule.location","ClassSchedule.class_fees","ClassSchedule.end_time","HealthMember.first_name","HealthMember.last_name"])->hydrate(false)->toArray();
		}
		$this->set("data",$data);
	}
	...
	public function addClass()
	{
		...
		$staff = $this->ClassSchedule->HealthMember->find("list",["keyField"=>"id","valueField"=>"name"])->where(["role_name"=>"staff_member"]);
	...
	public function viewSchedule()
	{
		...
		if($session["role_name"]=="member")
		{
			$classes_list = $this->ClassSchedule->HealthMemberClass->find()->where(["member_id"=>$session["id"]])->hydrate(false)->toArray();
