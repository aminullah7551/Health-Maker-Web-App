		...
		<div class="box-header">
			<section class="content-header">
			  ...
			  <?php
				...
				{ ?>
			  <ol class="breadcrumb">
				<a href="<?php echo $this->Health->createurl("StaffMembers","addStaff");?>" class="btn btn-flat btn-custom"><i class="fa fa-plus"></i> <?php echo __("Add Staff Member");?></a>
			  </ol>
			  <?php } ?>
			</section>
		</div>
		...
		<div class="box-body">
			<table class="mydataTable table table-striped" width="100%">
				...
				{				
					...
					echo "<td>{$row['health_role']['name']}</td>";
					...
					echo "<td>
						<a href='".$this->Health->createurl('StaffMembers','editStaff')."/{$row['id']}' class='btn btn-flat btn-primary' title='".__('Edit')."'><i class='fa fa-edit'></i></a>
						<a href='".$this->Health->createurl('StaffMembers','deleteStaff')."/{$row['id']}' class='btn btn-flat btn-danger' title='".__('Delete')."' onClick=\"return confirm('$confirmMsg')\"><i class='fa fa-trash-o'></i></a>
						</td>";
					}
					echo "</tr>";
				} 
