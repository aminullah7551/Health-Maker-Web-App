...
//	public $helpers = ['AkkaCKEditor.CKEditor'];
    ...
    public function beforeRender(Event $event)
    {
		if(file_exists(TMP.'installed.txt')) 
		{
			...
			$check_alert_on = $this->HEALTHFunction->getSettings("enable_alert");
			if($check_alert_on)
			{			
				$this->HEALTHFunction->sendAlertEmail();
				$this->HEALTHFunction->sendAlertEmailToAdmin();		
			}		
		}
 	...
 	public function beforeFilter(Event $event)
	{
		...
		{
			$this->loadComponent("HEALTHFunction");
			@$lang = $this->HEALTHFunction->getSettings("sys_language");		
			if (empty($lang)) {
				return;
			}
			...
			// @$time_zone = $this->HEALTHFunction->getSettings("time_zone");
			// if($time_zone != '')
			// {
