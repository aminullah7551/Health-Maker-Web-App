...
$is_rtl = $this->Health->getSettings("enable_rtl");
if ($is_rtl) {
...
$dtp_lang = $this->health->getSettings("datepicker_lang");
...
$(".validateForm").validationEngine();
	...
	$(".dob").datepicker({yearRange: "-100:+0",changeYear: true,changeMonth: true, dateFormat:"<?php echo $this->Health->dateformat_PHP_to_jQueryUI($this->Health->getSettings("date_format")); ?>" ,"language" : "<?php echo $dtp_lang; ?>"});
	//$(".datepick").datepicker({format: 'yyyy-mm-dd',"language" : "<?php echo $dtp_lang; ?>"});
	$(".datepick").datepicker({dateFormat:"<?php echo $this->Health->dateformat_PHP_to_jQueryUI($this->Health->getSettings("date_format")); ?>" ,"language" : "<?php echo $dtp_lang; ?>",});

	$(".content-wrapper").css("height","2600px");

	$(".mem_valid_from").datepicker({yearRange: "-100:+0",changeYear: true,changeMonth: true,dateFormat: "<?php echo $this->Health->dateformat_PHP_to_jQueryUI($this->Health->getSettings("date_format")); ?>" ,"language" : "<?php echo $dtp_lang; ?>"}).on("change",function(ev){
...
$radio = [
    ['value' => 'male', 'text' => 'Male' , 'style' => 'margin:0 10px;'],
    ['value' => 'female', 'text' => 'Female' , 'style' => 'margin:0 10px;'],
];
...
echo '<div class="col-md-6">';
echo $this->Form->input("", ["label" => false, "name" => "birth_date", "class" => "form-control dob validate[required] datepick", "value" => (($edit) ? date($this->Health->getSettings("date_format"), strtotime($data['birth_date'])) : ''), "onkeydown" => "return false"]);
echo "</div>";
echo "</div>";
...
echo '<div class="col-md-2">';
echo $this->Form->input("", ["label" => false, "name" => "membership_valid_from", "class" => "form-control validate[required] mem_valid_from", "value" => (($edit && $data['membership_valid_from'] != "") ? $this->Health->get_db_format(date($this->Health->getSettings("date_format"), strtotime($data['membership_valid_from']))) : '')]);
echo "</div>";
...
echo $this->Form->input("", ["type" => "hidden", "label" => false, "name" => "membership_valid_to", "class" => "form-control validate[required] valid_to", "value" => (($edit && $data['membership_valid_to'] != "") ? date("Y-m-d", strtotime($data['membership_valid_to'])) : ''), "readonly" => true]); ?>
			<input type='hidden' name='membership_valid_to' class='check' value='<?php ($edit && $data['membership_valid_to'] != "") ? $this->Health->get_db_format(date($this->Health->getSettings("date_format"), strtotime($data['membership_valid_to']))) : ''?>'>
			<?php echo "</div>";
echo "</div>";
...
// echo $this->Form->button(__("Checkout"),['class'=>"col-md-offset-2 btn btn-flat btn-success","id"=>"","name"=>"stripe_checkout"]);
echo "<a href='javascript:void(0)' id='checkout-button' class='btn btn-success' style='margin-left:10px'>" . __('Checkout') . "</a>";
echo "</div>";
...
?>
		...
		<input type="hidden" value="<?php echo $this->request->base; ?>/StripeSubscription/stripeSession" id="stripe_session_url">
		</div>
	</div>
</section>
 <script>
 var stripe = Stripe('pk_test_8xNuwYhDeaKJIGgIJ9OmCHa0');
 ...
 checkoutButton.addEventListener("click", function () {
	...
	var selectedMembershipId = $(".membership_id").val();
	
	var ajaxurl = $("#stripe_session_url").val();
	var curr_data = { data : data};
