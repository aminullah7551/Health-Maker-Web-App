...
{	
    ...
    public function paymentList() {	
		...
		if($session["role_name"] == "member") {			
			$data = $this->MembershipPayment->find("all")->contain(["Membership","HealthMember"])->where(["HealthMember.id"=>$session["id"]])->hydrate(false)->toArray();
		}else {
			$data = $this->MembershipPayment->find("all")->contain(["Membership","HealthMember"])->hydrate(false)->toArray();
		}
		$this->set("data",$data);
		if($this->request->is("post")) {			
			...
			if($this->request->data["payment_method"] == "Stripe" && $session["role_name"] == "member") {
				if(!empty($this->request->data['stripeToken'])) {
					...
					$member = $this->MembershipPayment->HealthMember->find()->where(["id"=>$mem_id])->hydrate(false)->toArray();
					...
					$stripe = array(
						"secret_key" => $this->HEALTHFunction->getSettings('stripe_secret_key'),
						"publishable_key" => $this->HEALTHFunction->getSettings('stripe_publishable_key') 
					);
					...
					$country = $this->HEALTHFunction->getSettings('country');
					...
					$currency = $this->HEALTHFunction->getSettings("currency");
			...
			{				
				...
				$custom_var = $mp_id;			
				$user_info = $this->MembershipPayment->HealthMember->get($user_id);
	...
	{
		$this->set("edit",false);
		$members = $this->MembershipPayment->HealthMember->find("list",["keyField"=>"id","valueField"=>"name"])->where(["role_name"=>"member"]);
		...
		{			
			...
			$start_date = $this->HEALTHFunction->get_db_format_date($this->request->data['membership_valid_from']);
			
			$end_date = $this->HEALTHFunction->get_db_format_date($this->request->data['membership_valid_to']);
			...
			$member_data = $this->MembershipPayment->HealthMember->get($mid);
			...
			$this->MembershipPayment->HealthMember->save($member_data);
    ...
    {
		$data = $this->MembershipPayment->HealthIncomeExpense->find("all")->contain(["HealthMember"])->where(["invoice_type"=>"income"])->hydrate(false)->toArray();
	...
	public function addIncome()
    {
		...
		if($this->request->is("post"))
		{	
			$row = $this->MembershipPayment->HealthIncomeExpense->newEntity();
			...
			$data["invoice_date"] = $this->HEALTHFunction->get_db_format_date($data['invoice_date']);	

			$row = $this->MembershipPayment->HealthIncomeExpense->patchEntity($row,$data);			
			if($this->MembershipPayment->HealthIncomeExpense->save($row))
			{
	...
	public function incomeEdit($eid)
	{
		...
		$row = $this->MembershipPayment->HealthIncomeExpense->get($eid);
    ...
    {
		$row = $this->MembershipPayment->HealthIncomeExpense->get($did);
		if($this->MembershipPayment->HealthIncomeExpense->delete($row))
		{
	...
	public function printInvoice()
	{
		...
		{
			$invoice_data = $this->MembershipPayment->find("all")->contain(["HealthMember","Membership"])->where(["mp_id"=>$id])->hydrate(false)->toArray();
		...
		{
			$income_data = $this->MembershipPayment->HealthIncomeExpense->find("all")->contain(["HealthMember"])->where(["HealthIncomeExpense.id"=>$id])->hydrate(false)->toArray();
		...
		{
			$expense_data = $this->MembershipPayment->HealthIncomeExpense->find("all")->where(["HealthIncomeExpense.id"=>$id])->select($this->MembershipPayment->HealthIncomeExpense);
			$expense_data = $expense_data->leftjoin(["HealthMember"=>"health_member"],
									["HealthIncomeExpense.receiver_id = HealthMember.id"])->select($this->MembershipPayment->HealthMember)->hydrate(false)->toArray();
			$expense_data[0]["health_member"] = $expense_data[0]["HealthMember"];
			unset($expense_data[0]["HealthMember"]);	
    ...
    {
		$data = $this->MembershipPayment->HealthIncomeExpense->find("all")->where(["invoice_type"=>"expense"])->hydrate(false)->toArray();
		$this->set("data",$data);
    }
	...
	public function addExpense()
    {
		...
		{	
			...
			$data["invoice_date"] = $this->HEALTHFunction->get_db_format_date($this->request->data['invoice_date']);	
			$row = $this->MembershipPayment->HealthIncomeExpense->patchEntity($row,$data);			
			if($this->MembershipPayment->HealthIncomeExpense->save($row))
			{
				$this->Flash->success(__("Success! Record Saved Successfully."));	
	...
	public function ipnFunction()
	{
		if($this->request->is("post"))
		{
			...
			//wp_mail( 'bhaskar@dasinfomedia.com', 'healthpaypal', $log_array);
			$row = $this->MembershipPayment->MembershipPaymentHistory->newEntity();
			$row = $this->MembershipPayment->MembershipPaymentHistory->patchEntity($row,$feedata);
			if($this->MembershipPayment->MembershipPaymentHistory->save($row))
			{
