...
{	
	...
	{			
...

		{
			...
			$ext = $this->HEALTHFunction->check_valid_extension($this->request->data('gmgt_membershipimage')['name']);

			if($ext != 0)

			{

				$new_name = $this->HEALTHFunction->uploadImage($this->request->data["gmgt_membershipimage"]);
	...
	public function activateOnStripe($mid)
	{
		...
		$stripeSecretKey = $this->HEALTHFunction->getGeneralSettingFieldValue("stripe_secret_key");
		if($stripeSecretKey == "YOUR SECRET KEY" || $stripeSecretKey == "")
		{
			...
			$stripeProductId = $this->HEALTHFunction->getGeneralSettingFieldValue("stripe_product_id");
			$currency = strtolower($this->HEALTHFunction->getGeneralSettingFieldValue("currency"));
