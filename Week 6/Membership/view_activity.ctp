		...
		<div class="box-body">
			...
			<table class="mydataTable table table-striped table-responsive" "width:100%;">
				...
				<?php
					...
					{
						$title = $this->Health->get_activity_name($activity['activity_id']);				
						$category = $this->Health->get_category_name($activity['activity']['cat_id']);				
						$staff = $this->Health->get_staff_name($activity['activity']['assigned_to']);				
						echo "<tr>
							<td>{$title}</td>
							<td>{$category}</td>
							<td>{$staff}</td>
							<td><a href='".$this->Health->createurl('Membership','deleteActivity')."/{$activity['id']}' class='btn btn-flat btn-danger'>".__('Delete')."</a></td>
							</tr>";
					}
				?>
				</tbody>
