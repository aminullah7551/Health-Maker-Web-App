...
<section class="content">
	...
	<div class="col-md-12 box box-default">
		<div class="box-header">
			...
			{ ?>
			  <ol class="breadcrumb">				
				<a href="<?php echo $this->Health->createurl("Membership","add");?>" class="btn btn-flat btn-custom"><i class="fa fa-plus"></i> <?php echo __("Membership");?></a>
			  </ol>
			  <?php } ?>
			</section>
		</div>
		...
		<div class="box-body">
			<table class="mydataTable table table-striped" width="100%">
				...
				<?php
					...
					{
						$duration = $this->Health->get_plan_duration($membership->install_plan_id);
						$durat = "";			
						if(empty($duration))
						{
							$duration["number"] = "";
						...
						<td>{$durat}</td>
						<td>". $this->Health->get_currency_symbol() ."{$membership->signup_fee}</td>";
						
						if($session["role_name"] != "member")	
						{
						echo "<td>";
						
						echo " <a href='{$this->Health->createurl("Membership","editMembership")}/{$membership->id}' title='".__('Edit')."' class='btn btn-flat btn-primary' ><i class='fa fa-edit'></i></a>
						<a title='".__('Delete')."' did='{$membership->id}' class='del-membership btn btn-flat btn-danger' data-url='".$this->Health->createurl("HealthAjax","deleteMembership")."'><i class='fa fa-trash-o'></i></a>";
						
						//if($session["role_name"] == "administrator")
						//{ 
						echo " <a href='{$this->Health->createurl("Membership","viewActivity")}/{$membership->id}' class='btn btn-flat btn-info'>".__("Activities")."</a>";	
						//}
						echo " <a href='{$this->Health->createurl("Membership","activateOnStripe")}/{$membership->id}' class='btn btn-flat btn-primary' >".__('Activate On Stripe')."</a>";
